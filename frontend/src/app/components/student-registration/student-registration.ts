  ...
  <div class="w-full md:w-5/12 flex flex-col space-y-8">
    ...
    <div class="relative group self-center md:self-start">
      <div class="artisanal-border w-48 h-48 rounded-full flex items-center justify-center bg-surface-container overflow-hidden border-2 border-primary/20">
        @if (fotoPerfilUrl) {
          <img [src]="fotoPerfilUrl" class="w-full h-full object-cover" alt="Previsualización">
        } @else {
        ...
        <button type="button" class="absolute bottom-2 right-2 bg-secondary text-on-secondary p-3 rounded-full shadow-lg hover:scale-105 transition-transform">
    ...
    <form (ngSubmit)="onSubmit()" class="space-y-8">
      <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <div class="group">
            <label class="block text-xs font-bold uppercase tracking-wider text-secondary mb-2" for="institution">Institución Educativa</label>
            <input [(ngModel)]="institution" name="institution" class="w-full bg-surface-variant/30 border-0 border-b-2 border-outline-variant focus:border-tertiary focus:ring-0 px-0 py-3 transition-colors text-on-surface placeholder:text-on-surface-variant/50 font-medium" id="institution" placeholder="Nombre de tu colegio" type="text" required/>
          </div>
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div class="group">
            <label class="block text-xs font-bold uppercase tracking-wider text-secondary mb-2" for="motherTongue">Lengua Materna</label>
            <input [(ngModel)]="motherTongue" name="motherTongue" class="w-full bg-surface-variant/30 border-0 border-b-2 border-outline-variant focus:border-tertiary focus:ring-0 px-0 py-3 transition-colors text-on-surface placeholder:text-on-surface-variant/50 font-medium" id="motherTongue" placeholder="Ej: Quechua, Aimara, Castellano" type="text" required/>
          </div>
          <div class="group">
        ...
        <div class="group">
          <label class="block text-xs font-bold uppercase tracking-wider text-secondary mb-2" for="bio">Biografía / Sobre ti</label>
          <textarea [(ngModel)]="bio" name="bio" rows="3" class="w-full bg-surface-variant/30 border-0 border-b-2 border-outline-variant focus:border-tertiary focus:ring-0 px-0 py-3 transition-colors text-on-surface placeholder:text-on-surface-variant/50 font-medium resize-none" id="bio" placeholder="Cuéntanos un poco de tu historia..." required></textarea>
        </div>

        <div class="group">
          <label class="block text-xs font-bold uppercase tracking-wider text-secondary mb-2" for="photo">URL de Foto de Perfil (Opcional)</label>
          <input [(ngModel)]="fotoPerfilUrl" name="photo" class="w-full bg-surface-variant/30 border-0 border-b-2 border-outline-variant focus:border-tertiary focus:ring-0 px-0 py-3 transition-colors text-on-surface placeholder:text-on-surface-variant/50 font-medium" id="photo" placeholder="https://ejemplo.com/mifoto.jpg" type="url"/>
        </div>
      </div>
